...
//验证密码
    boolean checkPasswd(long sellerId, String passwd);
    //在修改信息时候判断是否输入正确密码
  ...
  */
    ...
    //获得广告数据库
    LinkedList<String> getSellerAdvertisement(long sellerId);
    //返回一连串的广告的图片路径

    //写入要发布的广告
    boolean writeInAdvertisment(long sellerId, LinkedList<String> paths);

//逼哥再考虑一下广告收费的接口，这个我没考虑

    //创建商品
    boolean createProductInformation(long sellerId, ProductInformation productInfo);
    //商品信息
  /*
  {
  	seller		:	long,
  ...
  */
    ...
    //获取商家所有商品
    LinkedList<ProductInformation> getAllProducts(long sellerId);

    //删除某一个商品
    boolean deleteProducts(long productID);

    //为某个商品修改信息
    boolean changedProducts(ProductInformation newProduct);

    //获取订单信息
    LinkedList<Orderinfo> getTrade(long sellerID, int type);
    //type : 0 :所有的
    //1：完成的交易
    //2：所有的交易
